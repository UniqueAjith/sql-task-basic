...
/*Return properties of only studio that are sony*/
SELECT * FROM topmovies WHERE Studio LIKE "%sony%";

/*Return count of sony studio*/
SELECT COUNT(*) AS total_movie FROM topmovies WHERE Studio LIKE "%sony%";

/*Return title of sony studio and fox studio*/
SELECT Title FROM topmovies WHERE Studio LIKE "%sony" OR Studio LIKE "%Fox%";

/*Return the avg of Worldwide*/
SELECT ROUND(AVG(WorldWide)) AS total_collection FROM topmovies;

/*Return Title name using having*/
SELECT Title FROM topmovies GROUP BY Studio HAVING Domestic > 250; 

/*Return hit,blockbluser and mega blockbluster as count*/
SELECT COUNT(*),
    CASE
        WHEN WorldWide < 1000 THEN "HIT"
        WHEN WorldWide < 2000 THEN "BLOCKBLUSTER"
        WHEN WorldWide > 2000 THEN "MEGA BLOCKBLUSTER"
    END AS "Box_Office"
FROM topmovies
GROUP BY Box_Office
